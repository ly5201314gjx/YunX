...
import com.yunx.cloud.data.db.AppDatabase
import com.yunx.cloud.data.db.DownloadTaskEntity
import com.yunx.cloud.data.download.ChunkDownloader
import com.yunx.cloud.data.download.DownloadManager
import com.yunx.cloud.data.download.DownloadNotifier
import com.yunx.cloud.data.backup.AuthBackupManager
import com.yunx.cloud.data.network.BaiduApi
import com.yunx.cloud.data.network.C139Api
import com.yunx.cloud.data.network.Pan123Api
import com.yunx.cloud.data.network.QuarkApi
import com.yunx.cloud.data.network.UCApi
import com.yunx.cloud.data.network.XunleiApi
...
import com.yunx.cloud.ui.components.GlassBottomBar
import com.yunx.cloud.ui.components.GlassBottomBarItem
import com.yunx.cloud.ui.components.IosAlertDialog
import com.yunx.cloud.ui.components.LocalGlassSurfaceBackdrop
import com.yunx.cloud.ui.components.LocalScreenBackdrop
...
import com.yunx.cloud.ui.components.liquidGlassSupported
import com.yunx.cloud.ui.components.topBarLiquidGlass
import com.yunx.cloud.ui.login.BaiduLoginScreen
import com.yunx.cloud.ui.login.C139LoginScreen
import com.yunx.cloud.ui.login.Pan123LoginScreen
import com.yunx.cloud.ui.login.QuarkLoginScreen
import com.yunx.cloud.ui.login.UCLoginScreen
import com.yunx.cloud.ui.login.XunleiLoginScreen
import com.yunx.cloud.ui.login.XunleiVerifyWebViewScreen
import com.yunx.cloud.ui.navigation.MainTab
import com.yunx.cloud.ui.screens.AboutScreen
import com.yunx.cloud.ui.screens.BookmarkScreen
import com.yunx.cloud.ui.screens.DownloadScreen
import com.yunx.cloud.ui.screens.DriveScreen
import com.yunx.cloud.ui.screens.OnboardingScreen
import com.yunx.cloud.ui.screens.ResolveScreen
import com.yunx.cloud.ui.screens.SettingsScreen
import com.yunx.cloud.ui.screens.ThemeScreen
import com.yunx.cloud.ui.screens.UpdateDialog
...
fun MainScreen() {
    ...
    val downloadManager = remember {
        DownloadManager(
            ...
            // 下载完成/失败提醒：独立通知（设置页控制开关/铃声）
            completeNotifier = { success, name, err ->
                DownloadNotifier.notify(context, success, name, err)
            }
        )
    }
    ...
    // 首次下载任务启动：请求通知权限（Android 13+，下载完成提醒可见性）并引导电池优化豁免一次。
    // 监听任务状态而非 downloadStarted，覆盖解析页/网盘页/手动添加等所有下载入口。
    val notificationPermissionLauncher = rememberLauncherForActivityResult(
        ActivityResultContracts.RequestPermission()
    ) { /* 拒绝仅影响通知，不阻断下载 */ }
    var notificationGuideShown by remember { mutableStateOf(false) }
    LaunchedEffect(Unit) {
        downloadViewModel.tasks.collect { tasks ->
            ...
            if (hasActive) {
                // Android 13+ 首次下载请求通知权限（下载完成提醒/前台通知可见）
                if (!notificationGuideShown) {
            ...
            if (!batteryGuideShown && hasActive) {
    ...
    // ===== 液态玻璃（Liquid Glass）采样源：背景层 + 内容层分离 =====
    // 复刻 legado-with-MD3 的 AppScaffold 机制：
    //  - backgroundBackdrop 记录纯背景色，供页面内玻璃卡片采样（避免自采样）；
    //  - contentBackdrop   记录页面内容，与背景合并成 screenBackdrop，
    //    供顶栏玻璃胶囊、底部玻璃导航栏采样（它们在内容层之外，不会自采样）。
    // 采样背景色在 lambda 外计算（rememberLayerBackdrop 的内容 lambda 非 @Composable，不能内联读取 MaterialTheme）
    val backgroundBaseColor = MaterialTheme.colorScheme.background
    val backgroundBackdrop = rememberLayerBackdrop {
        drawRect(backgroundBaseColor)
    }
    val contentBackdrop = rememberLayerBackdrop { drawContent() }
    val screenBackdrop = rememberCombinedBackdrop(backgroundBackdrop, contentBackdrop)

    CompositionLocalProvider(
        LocalScreenBackdrop provides screenBackdrop,
        LocalGlassSurfaceBackdrop provides backgroundBackdrop,
    ) {
    // 主框架与全屏覆盖层（关于页）放在同一 Box：覆盖层带过渡动画
    Box(modifier = Modifier.fillMaxSize()) {
    // 背景采样层（置于最底层）：录制纯背景色，供玻璃组件采样
    Box(
        modifier = Modifier
            .fillMaxSize()
    ...
    val topBarContent: @Composable () -> Unit = {
        if (currentTab == MainTab.Resolve) {
            // 解析页：顶部收敛为轻量操作栏，仅保留右侧书签入口（透明背景），
            // 主标题/副标题/3D 插画由页面 Hero 区自行承载，避免标题叠层。
            TopAppBar(
                title = {},
                actions = {
                    IconButton(
                        onClick = { showBookmarks = true },
                        // 液态玻璃胶囊（复刻 legado TopBarLiquidGlass：真模糊+折射+高光+按压律动）
                        modifier = Modifier.topBarLiquidGlass(RoundedCornerShape(50))
                    ) {
            ...
            // 网盘 / 下载 / 设置：隐藏标题栏（iOS 极简），仅保留状态栏安全区占位，
            // 让页面内容直接上移到状态栏下方，不再被标题栏挤压。
            Box(
                modifier = Modifier
                    .fillMaxWidth()
                    .statusBarsPadding()
        ...
        AnimatedContent(
            ...
            saveableStateHolder.SaveableStateProvider(tab) {
                when (tab) {
                    ...
                    MainTab.Download -> DownloadScreen(downloadViewModel, driveQuotaViewModel)
    ...
    if (isLandscape) {
        ...
        // 竖屏：Scaffold + 悬浮液态玻璃底部导航栏
        // 底部导航移出 Scaffold bottomBar 槽位，改为内容之上的悬浮覆盖层（复刻 legado FloatingBottomBar）：
        // 内容延伸到导航栏后方，玻璃导航栏对其做真实模糊/折射，而非假半透明。
        Scaffold(
            modifier = Modifier.fillMaxSize(),
            snackbarHost = {
                SnackbarHost(
                    hostState = snackbarHostState,
                    // 悬浮导航栏不占位，snackbar 底部留白避免被遮挡
            ...
            Box(
                modifier = Modifier
                    ...
                    .padding(
                        ...
                        // 内容延伸到悬浮玻璃导航栏后方（真实模糊的前提），仅保留手势条内边距
        ...
        // 底部悬浮液态玻璃导航栏（覆盖在内容之上，采样 screenBackdrop）
        MainBottomBar(
            currentTab = currentTab,
            onTabSelected = { currentTab = it },
            modifier = Modifier.align(Alignment.BottomCenter),
    ...
    // 关于析盘：叠加覆盖层（淡入 + 轻微缩放过渡）
    AnimatedVisibility(
        visible = showAbout,
        enter = fadeIn(NativeSpringSoft) + scaleIn(NativeSpring, initialScale = 0.96f),
        exit = fadeOut(NativeSpringSoft) + scaleOut(NativeSpring, targetScale = 0.96f),
        modifier = Modifier.fillMaxSize()
    ) {
        AboutScreen(
    ...
    if (showBatteryGuide) {
        IosAlertDialog(
            ...
            title = "保持后台下载",
            message = "「锁屏后保持下载」已开启，但应用尚未加入「忽略电池优化」白名单，息屏后可能被系统中断下载。是否前往系统设置？",
            confirmText = "前往设置",
            onConfirm = {
 ...
 * 悬浮液态玻璃胶囊条（复刻 legado FloatingBottomBar）：真模糊 + 折射 + 动态高光/内阴影，
 * 指示胶囊可拖拽吸附，拖拽时带液态缩放与速度惯性；选中项点亮主题色。
 */
@Composable
    ...
    GlassBottomBar(
        modifier = modifier
            ...
            .padding(
                ...
                // 悬浮玻璃导航栏避开系统手势条/三键导航（复刻 legado FloatingBottomBar 的 inset 处理）
                bottom = 12.dp + WindowInsets.navigationBars
                    .asPaddingValues()
                    .calculateBottomPadding()
            ),
        selectedIndex = { currentTab.ordinal },
        onSelected = { onTabSelected(MainTab.values()[it]) },
        backdrop = backdrop,
        tabsCount = MainTab.values().size,
        isBlurEnabled = liquidGlassSupported,
    ) {
